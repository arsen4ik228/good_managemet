...
const MobilePolicy = () => {
  ...
  useEffect(() => {
    ...
    setInputValue(
      currentPolicy.policyName === "Политика"
        ? `Политика №${currentPolicy.policyNumber}`
        : currentPolicy.policyName
    );
    currentPolicy.type === "Инструкция"
      ? setValueType("Инструкция")
      : setValueType("Директива");
  ...
  return (
    <>
      <div className={classes.wrapper}>
        ...
        {!disabled && (
          <ButtonContainer clickFunction={saveUpdatePolicy}>
            сохранить
          </ButtonContainer>
        )}
      </div>
      ...
      <HandlerMutation
        ...
        Error={
          isErrorUpdatePoliciesMutation && localIsResponseUpdatePoliciesMutation
        }
        Success={
          isSuccessUpdatePoliciesMutation &&
          localIsResponseUpdatePoliciesMutation
        ...
        textError={
          ErrorUpdatePoliciesMutation?.data?.errors?.[0]?.errors?.[0]
            ? ErrorUpdatePoliciesMutation.data.errors[0].errors[0]
      ...
      {openAlertModal && (
        <AlertUpdateData setModalOpen={setOpenAlertModal}></AlertUpdateData>
      )}
    </>
  );
};
