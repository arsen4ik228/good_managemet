...
export default function Statistics() {
  ...
  useEffect(() => {
    // Установка organizationId для поиска постов и отчётного дня
    if (
      Object.keys(currentStatistic).length > 0 &&
      allPosts.length > 0 &&
      ...
      if (orgId) {
        ...
        const report = organizations?.filter((item) => item?.id === orgId);
    ...
    if (statisticDatas.length > 0 && reportDay) {
      ...
      if (typeGraphic === "Ежедневный" && day !== "") {
        ...
        const monthlyData = statisticDatas.reduce((acc, item) => {
          ...
          const monthKey = `${itemDate.getFullYear()}-${
            itemDate.getMonth() + 1
          }`; // Год-месяц как ключ
          if (
            !isNaN(itemDate) &&
        ...
        <div className={classes.menuContent}>
          <div className={classes.nameSection}>
            ...
            <img
              ...
              style={{
                transform: !openMenu ? "rotate(270deg)" : "rotate(90deg)",
              }}
            />
          </div>
          {openMenu && (
            <>
              ...
              <div className={classes.menuContainer}>
                <div className={classes.name}>Пост:</div>
                <div className={classes.selectSection}>
                  <select
                    value={
                      postId !== "null" ? postId : currentStatistic?.post?.id
                    }
                    onChange={(e) => {
              ...
              <div className={classes.menuContainer}>
                <div className={classes.name}>Тип графика:</div>
                <div className={classes.selectSection}>
                  <select
                    value={type !== "null" ? type : currentStatistic.type} // Устанавливаем ID, по умолчанию пустая строка
                    onChange={(e) => {
              ...
              <div className={classes.tableContainer}>
                <div className={classes.tableDots}>
                  <div className={classes.tableButton}>
                    <div onClick={addPoint} className={classes.addedBtn}>
                      Добавить
                    </div>
                    <div onClick={deletePoint} className={classes.removeBtn}>
                      Удалить
                    </div>
                  </div>
                  ...
                  {createPoints
                    ...
                    ?.map((item, index) => {
                      if (item.valueDate === "") {
                        item.valueDate = new Date().toISOString().split("T")[0];
                      }
                      return (
              ...
              <div className={classes.menuContainer}>
                <textarea
                  ...
                  value={description || currentStatistic.description}
                  ...
                  <td>
                    {showPoints?.map((item) => (
                      <div key={item.id} className={classes.row}>
                        <span className={`${classes.date} ${classes.textGrey}`}>
