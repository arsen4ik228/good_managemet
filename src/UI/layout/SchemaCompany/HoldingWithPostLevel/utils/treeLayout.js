    ...
    function positionNode(node, depth = 0, left = 0, right = 0) {
        ...
        const currentNode = {
            ...
            data: {
                ...
                userName: node?.posts[0]?.user?.firstName + ' ' + node?.posts[0]?.user?.secondName || 'Нет пользователя', 
                postName: node?.posts[0]?.postName,
                avatarUrl : node?.posts[0]?.user?.avatar_url,
                original: node
            },
            style: {
                width: nodeWidth,
